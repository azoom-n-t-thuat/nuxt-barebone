<template>
  <div class="contact-logs-info-card">
    <div class="mdc-card">
      <div class="info -name">
        {{ contactStatuses[contactLogsValue] || contactLogsValue }}
      </div>
      <div class="info -amount">
        <span class="number">{{ contactLogsKey }}</span> 件
      </div>
    </div>
  </div>
</template>
...
<script>
export default {
  props: ['contactLogsKey', 'contactLogsValue'],
  data() {
    return {
      contactStatuses: {
        receptionist: '受付',
